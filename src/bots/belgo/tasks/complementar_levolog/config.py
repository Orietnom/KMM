...
CONCURRENCY_POLICY = task.policies.ConcurrencyPolicy(
    value=os.getenv("TASK_POLICY_BELGO_LEVOLOG_CONCURRENCY", "1")
)
if CONCURRENCY_POLICY.value != 1:
    raise ValueError("O worker BELGO Levolog exige concorrência 1 por usar uma sessão KMM")

WORKER_POLICY = task.policies.ConsumerPolicy(
    name="worker",
    loop=task.policies.ConsumerLoopPolicy(
        concurrency=CONCURRENCY_POLICY,
        limit=os.getenv("TASK_POLICY_BELGO_LEVOLOG_LIMIT") or None,
        streaming=False,
    ),
    fetch=task.policies.FetchPolicy(
        ...
        batch=task.policies.BatchPolicy(
            size=os.getenv("TASK_POLICY_BELGO_LEVOLOG_BATCH_SIZE", "10")
        ),
    ),
    transaction_runtime=task.policies.TransactionRuntimePolicy(
        timeout=os.getenv("TASK_POLICY_BELGO_LEVOLOG_TIMEOUT", "1800")
    ),
...
TASK_BELGO_LEVOLOG_COMPLEMENT = task.TaskConfig(
    name="belgo-worker-complementar-levolog",
    task=TaskBelgoLevologComplement,
...
task.manager.register(TASK_BELGO_LEVOLOG_COMPLEMENT)


def run() -> None:
    task.manager.run("belgo-worker-complementar-levolog", debug=True, worker_id=0)
